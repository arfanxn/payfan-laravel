...
class RejectingRequestMoneyNotification extends Notification implements ShouldBroadcast,  ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        ...
        return (new MailMessage)
            ->subject('Rejecting request money from "'
                . substr($this->order->fromWallet->user->name, 0, 10)  . '" | ' . config('app.name'))
            ->greeting("Hello, $notifiable->name .")
            ->line('You rejecting a request money from "' . $this->order->fromWallet->user->name . '", 
            ...
            ->line('Total : ' . $total)
            ->line('Rejected at : ' . Carbon::parse($this->order->updated_at)->toDateTimeString() . " UTC")
    ...
    {
        return [
            "header" => "Rejecting request money",
            "body" =>
            'You rejecting a request money from "' . $this->order->fromWallet->user->name . '", 
                amount ' . $this->order->amount . " $",
            "action" => [
